...
portable_icon_path="${PORTABLE_APP_ICON_PATH:-$repo_root/qt/resources/icons/app.ico}"
package_script="$repo_root/scripts/package_qt_win64.sh"
...
if [[ ! -f "$portable_icon_path" ]]; then
    echo "Portable launcher icon not found: $portable_icon_path" >&2
    exit 1
fi
if [[ ! -x "$package_script" ]]; then
    echo "Packaging script not executable: $package_script" >&2
    exit 1
fi
...
APP_VERSION="$app_version" PACKAGE_QT_WIN64_MODE=portable PORTABLE_APP_ICON_PATH="$portable_icon_path" "$package_script" "$build_dir"
